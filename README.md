...
-  **Niezawodny / Reliable:** Retry logic dla czujników, RTC fallback, thread-safe
-  **Monitorowanie baterii / Battery monitoring:** Napięcie + procent rozładowania
-  **Dokładne pomiary / Accurate measurements:** SHT3x (±0.3°C, ±2% RH), timeout + walidacja zakresu
...
> - Rzeczywisty czas pracy zależy od / Actual runtime depends on:
...
-  Czy wybrano `Tools → Zigbee mode → Zigbee ED`?  WYMAGANE / REQUIRED
...
-  Sieć Zigbee mogła zmienić polling interval / Zigbee network may have changed polling interval
